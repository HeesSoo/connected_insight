...
import HtmlRenderer from "@/app/[lang]/(web)/(main)/_component/HtmlRenderer";
...
export interface ProductDetail {
    ...
    category_file?: CategoryFile | null;
    category_table?: string;
    category_2_file?: CategoryFile[] | null;
    fov?: number;
    resolution?: number;
    dof?: number;
    wd?: number;
        ...
        <div className="w-[712px] max-md:w-full">
            <div className="relative">
                ...
                >
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <g mask="url(#mask0_2067_11491)">
                            <path d="M7.373 12.751H18.75C18.9628 12.751 19.141 12.6792 19.2845 12.5355C19.4282 12.392 19.5 12.2138 19.5 12.001C19.5 11.7881 19.4282 11.61 19.2845 11.4665C19.141 11.3228 18.9628 11.251 18.75 11.251H7.373L12.5423 6.08173C12.6909 5.93307 12.7643 5.75907 12.7625 5.55973C12.7605 5.3604 12.682 5.18315 12.527 5.02798C12.3718 4.88315 12.1962 4.80815 12 4.80298C11.8038 4.79782 11.6282 4.87282 11.473 5.02798L5.13275 11.3682C5.03908 11.4619 4.97308 11.5606 4.93475 11.6645C4.89625 11.7683 4.877 11.8805 4.877 12.001C4.877 12.1215 4.89625 12.2336 4.93475 12.3375C4.97308 12.4413 5.03908 12.5401 5.13275 12.6337L11.473 18.974C11.6115 19.1125 11.783 19.1833 11.9875 19.1865C12.192 19.1897 12.3718 19.1188 12.527 18.974C12.682 18.8188 12.7595 18.6406 12.7595 18.4395C12.7595 18.2381 12.682 18.0599 12.527 17.9047L7.373 12.751Z" fill="white" />
                        </g>
                    </svg>
                </button>
                ...
                <button
                    ...
                    onClick={() => {
                        if (thumbnailIndex === thumbnails.length - 1) return;
                        swiper?.slideNext();
                    }}
                >
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="hidden max-md:block">
                    <div className="absolute -bottom-[20px] left-1/2 transform -translate-x-1/2 flex gap-3">
                        {thumbnails.map((_, idx) => (
                            <div
                                key={idx}
                                className={`w-2 h-2 ${idx === thumbnailIndex ? "bg-ePrimary" : "bg-g200"}`}
                ...
                {thumbnails.map((v, idx) => (
                    <div
                        ...
                        className={`w-full aspect-square bg-[#EFEFEF] flex justify-center items-center cursor-pointer ${
                            thumbnailIndex === idx ? "border border-ePrimary" : ""
                        }`}
    ...
    if (!isCis) {
        return (
            <div className="max-w-[1488px] px-6 w-full mx-auto my-[80px] max-md:mt-0 max-md:mb-[96px] max-md:px-5">
                ...
                {/* Row 3: category_file (단일 이미지) */}
                {data.category_file?.s3_url && (
                    <section className="mt-[80px] max-md:mt-14">
                        <Image
                            src={data.category_file.s3_url}
                            alt={data.category_file.name || data.name}
                            width={1488}
                            height={800}
                ...
                {/* Row 5~: category_2_file (이미지 배열) */}
                {data?.category_2_files && data?.category_2_files?.length > 0 && (
                    <section className="mt-[80px] max-md:mt-14 flex flex-col gap-[80px] max-md:gap-14">
                        {data?.category_2_files?.map((file, idx) => (
                            <Image
                                key={idx}
                                src={file.s3_url}
