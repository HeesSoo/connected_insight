            ...
            <div className="w-[955px] flex flex-col gap-6 max-md:gap-3">
                {downloadMap.map((v, index) => {
                    return v ? (
                        <div
                            key={index}
                            className="w-full flex justify-between items-center pb-6 border-b border-g200 max-md:pb-3"
                        >
                            <div>
                                <div className="text-sm font-medium text-g400 mb-1 max-md:text-xs max-md:leading-[18px]">
                                    {formatDateToYYMMDD(v.created_at)} Updated
                                </div>
                                <h3 className="text-large font-bold text-g950 max-md:text-base">
                                    {type[v.type]}
                                </h3>
                            </div>
                            <DownloadButton
                                file={{ url: v.s3_url, name: v.name }}
                            />
                        </div>
                    ) : null;
                })}
