...
export async function fetchTranslationsFromSheet(): Promise<TranslationData> {
  try {
    ...
    allSheets.forEach((sheet) => {
      ...
      const header = headerRow.map((cell) => cell.formattedValue || '');
      ...
      rowData.slice(1).forEach((row) => {
        ...
        const values = cells.map((cell) => cell.formattedValue || '');
        const [key, ...translationValues] = values;

        if (key) {
          languages.forEach((lang, index) => {
            if (lang && translationValues[index]) {
              translations[lang][key] = translationValues[index];
            }
          });
        }
      });
